  ...
  const apiFetch = async (endpoint, options = {}) => {
    if (API_BASE && API_BASE.startsWith('http')) {
      try {
        ...
        console.warn(`Requisição ${endpoint} falhou via fetch, usando fallback Supabase direto...`, err);
      }
    }
...
async function handleSupabaseFallback(endpoint, options = {}) {
  ...
  if (endpoint === '/api/license/activate' && method === 'POST' && bodyData) {
    ...
    // Obter dados da loja para validar o CNPJ
    const { data: lojaDb } = await supabase.from('loja').select('*').limit(1).single();
    const saved = localStorage.getItem('restaurant_loja');
    const lojaInfo = lojaDb || (saved ? JSON.parse(saved) : null);

    const cnpj = lojaInfo?.cnpj || '00.000.000/0001-00';
    ...
    const palavraSecreta = 'MenuChefTopSeguro2024';
    ...
    function gerarChaveMensal(cnpjVal, mesAnoVal, diasVal, moduloVal, palavraSecretaVal) {
      const cnpjLimpoVal = cnpjVal.replace(/\D/g, '');
      const moduloNormalizadoVal = (moduloVal || 'BASICO').toUpperCase();
      const misturada = `${cnpjLimpoVal}-${mesAnoVal}-${diasVal}-${moduloNormalizadoVal}-${palavraSecretaVal}`;
      const hash = md5(misturada);
    ...
    let resultado = { valida: false, dias: 0, modulo: null };
        ...
        for (const dias of diasOpcoes) {
          for (const modulo of modulosOpcoes) {
            ...
            if (chaveNormalizada === chaveGerada) {
              resultado = { valida: true, dias, modulo };
              break;
            }
          }
          if (resultado.valida) break;
        }
        if (resultado.valida) break;
      }
      if (resultado.valida) break;
    ...
    if (!resultado.valida) {
      return json({ message: 'Chave inválida ou não corresponde ao mês atual.' }, 400);
    }
    ...
    const { error: upsertError } = await supabase.from('licenses').upsert({
      ...
      dias_licenciados: resultado.dias,
      modulo: resultado.modulo,
      emergencia_usada_este_mes: ''
    }, { onConflict: 'id' });

    if (upsertError) {
      console.error('[Licença] Erro ao salvar licença no Supabase:', upsertError);
      return json({ message: 'Chave válida, mas erro ao salvar no banco de dados: ' + upsertError.message }, 500);
    }
  ...
  if (endpoint === '/api/license/emergency' && method === 'POST') {
    ...
    const { error: emergError } = await supabase.from('licenses').upsert({
      id: 1,
      vencimento: dataAtual.toISOString(),
      emergencia_usada_este_mes: mesAtual
    }, { onConflict: 'id' });
    if (emergError) {
      console.error('[Licença] Erro ao salvar emergência no Supabase:', emergError);
      return json({ message: 'Erro ao salvar prazo de emergência: ' + emergError.message }, 500);
    }
